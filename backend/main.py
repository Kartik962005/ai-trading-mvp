...
async def ai_chat(request: Request, body: ChatRequest):
    try:
        from app.strategies.nlp_backtester import get_client
        import re
        import json
        ...
        cls_resp = groq_client.chat.completions.create(
            ...
            messages=[{
                "role": "user",
                "content": f"""Classify this message into ONE intent. Reply ONLY with valid JSON, nothing else.
Intents:
- "price_query" → user asking about stock price on a specific date
- "backtest"    → user wants to test a trading strategy
- "general"     → anything else

Message: "{message}"

Reply format: {{"intent": "...", "date": "YYYY-MM-DD or null"}}"""
            }],
            temperature=0.0,
        )
        raw = cls_resp.choices[0].message.content.strip()
        raw = re.sub(r"```[\w]*", "", raw).replace("```", "").strip()
        try:
            cls = json.loads(raw)
        except Exception:
            m = re.search(r'\{[^}]+\}', raw, re.DOTALL)
            cls = json.loads(m.group()) if m else {"intent": "backtest", "date": None}
        ...
        if intent == "price_query":
            ...
            # If LLM didn't extract date, ask it again
            if not date_str or date_str == "null":
                date_resp = groq_client.chat.completions.create(
                    model="llama-3.3-70b-versatile",
                    messages=[{
                        "role": "user",
                        "content": f"Extract the date from this message and return ONLY a date in YYYY-MM-DD format. If no specific date, return null. Message: '{message}'"
                    }],
                    temperature=0.0,
                )
                date_str = date_resp.choices[0].message.content.strip().replace('"', '').strip()
            ...
            if not row.empty:
                ...
                msg_out = f"No trading data found for that date (may be a weekend or holiday)."
                if date_str and date_str not in ("null", "None", ""):
                    try:
                        target2 = pd.to_datetime(date_str)
                        df['_diff'] = abs(df['date'] - target2)
                        nearest = df.loc[df['_diff'].idxmin()]
                        msg_out = (
                            f"No data for that exact date (weekend/holiday). "
                            f"Nearest trading day: {str(nearest['date'].date())} — "
                            f"Open ₹{round(float(nearest['open']),2)} | "
                            f"Close ₹{round(float(nearest['close']),2)}"
                        )
                    except Exception:
                        pass
                return {"type": "price_query", "found": False, "message": msg_out}
        ...
        # ── Step 4: General AI answer ─────────────────────────────────────────
            ...
            gen_resp = groq_client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": (
                        f"You are SignalX, a professional AI trading assistant. "
                        f"Current stock: {ticker} | Price: ₹{cur} | RSI(14): {rsi} | "
                        f"SMA50: ₹{sma50} | SMA200: ₹{sma200}. "
                        f"Answer concisely. Use ₹ for prices."
                    )},
            ...
            return {
                "type":    "general",
                "message": gen_resp.choices[0].message.content.strip()
            }
